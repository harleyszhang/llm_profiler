...
class CountCausalLMMemory(object):
    ...
    def count_memory_weights(self, embedding_dtype_bytes: int = BYTES_FP16):
        ...
        memory_weight_per_layer = (
            (params_per_layer) * self.bytes_per_param
        )
    ...
    def count_memory_act_per_layer_attn_kernel(
        ...
        self_attn_kernel_memory_access = qk_matmul_load_act + qk_matmul_store_act + \
                                         sv_matmul_load_act + sv_matmul_store_act + \
                                         softmax_loat_act + softmax_store_act
    ...
    def count_memory_access_per_layer_kv_cache(
        ...
        if not flash_attn:
            qk_matmul_load_k_cache=(self.s+generate_len) * self.head_dim * bs * self.num_key_value_heads * kv_cache_bytes
            sv_matmul_load_v_cache=(self.s+generate_len) * self.head_dim * bs * self.num_key_value_heads * kv_cache_bytes
            store_k_cache_k_linear=(self.num_key_value_heads * self.head_dim * bs * seq_len*kv_cache_bytes)
            store_v_cache_v_linear=(self.num_key_value_heads * self.head_dim * bs * seq_len*kv_cache_bytes)
   
            if seq_len == 1:
                kv_cache_memory_access = qk_matmul_load_k_cache + sv_matmul_load_v_cache + store_k_cache_k_linear + store_v_cache_v_linear
            else:
    ...
    def count_memory_access_per_layer_qkvo_proj(
        ...
        if is_inference:
            ...
            qkvo_proj_load_act = 4 * bs * seq_len * self.hidden_size * qkvo_proj_dtype_bytes
            qkvo_proj_store_act = 4 * bs * seq_len * self.hidden_size * qkvo_proj_dtype_bytes
    ...
    def count_memory_access_per_layer_mlp(
        ...
        if is_inference:
            mlp_act_memory_per_gpu = max(load_act_gate_proj + store_act_gate_proj, 
                                         load_act_gelu + store_act_gelu, 
                                         load_act_down_proj + store_act_down_proj)
        else:
            mlp_act_memory_per_gpu = (load_act_gate_proj + store_act_gate_proj, 
                                    load_act_gelu + store_act_gelu, 
                                    load_act_down_proj + store_act_down_proj)
    ...
    def count_memory_access_per_layer_rmsnorm(
        ...
        rn_memory_access_per_gpu = rmsnorm_load_weight + rmsnorm_load_act + rmsnorm_store_act

        return rn_memory_access_per_gpu
    
    def count_memory_access_input_embedding(self, bs: int,
                                              seq_len: int) -> float:
        input_embedding_load_act = bs * seq_len * self.act_dtype_bytes 
        input_embedding_store_act = bs * seq_len * self.hidden_size * self.act_dtype_bytes
        input_embedding_memory_access_per_gpu = input_embedding_load_act + input_embedding_store_act
        
        return input_embedding_memory_access_per_gpu
    
    def count_memory_act_output_embedding(self, bs: int,
                                               seq_len: int) -> float:
        output_embedding_act_memory_per_gpu = self.Vocab_size * bs * seq_len * self.act_dtype_bytes
    ...
    def count_memory_act_per_layer(
        ...
        act_mem_per_layer_qkvo_proj = self.count_memory_access_per_layer_qkvo_proj(
                bs,
                seq_len,
                is_inference,
        ...
        act_mem_per_layer_mlp = self.count_memory_access_per_layer_mlp(
                bs,
                seq_len,
                is_inference,
        ...
        act_mem_per_layer_rn = self.count_memory_access_per_layer_rmsnorm(
                bs,
                seq_len,
            ) / 2 
        ...
        if is_inference:
            # act = memory_access // 2
            act_memory_per_layer = max(act_mem_per_layer_qkvo_proj,
    ...
    def count_memory_kv_cache_per_layer(
        ...
        memory_kv_cache_per_layer = bs * (seq_len + generate_len) * num_kv_heads_per_gpu * self.head_dim * 2 * kv_cache_bytes
