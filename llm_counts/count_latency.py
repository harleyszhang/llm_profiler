from .utils.config import LLMConfigs, get_gpu_hbm_bandwidth, get_intra_node_bandwidth, get_TFLOPS_per_gpu
...
class CountCausalLMLatency(object):
    ...
    def __init__(self, llm_configs: LLMConfigs, data_type="fp16") -> None:
        ...
        self.llm_flops = CountCausalLMFlops(self.model_config, self.b, self.o, self.tp_size)
    ...
    def common_count_latency_for_ops(
        ...
        if ops_type=="qkvo_proj":
            flops = self.llm_flops.count_flops_per_layer_qkvo_proj(bs, seq_len)  / self.tp_size
            weight_memory = self.llm_params.count_params_per_layer_mha() * self.bytes_per_param  / self.tp_size
            memory_access = self.llm_memory.count_memory_access_per_layer_qkvo_proj(
                bs, seq_len, is_inference
            ) / self.tp_size
        elif ops_type=="attn_kernel":
            flops = self.llm_flops.count_flops_per_layer_attn_kernel(bs, seq_len) / self.tp_size
            weight_memory = 0
            memory_access = self.llm_memory.count_memory_act_per_layer_attn_kernel(
            ...
            weight_memory = self.llm_params.count_params_per_layer_mlp() * self.bytes_per_param  / self.tp_size
            memory_access = self.llm_memory.count_memory_access_per_layer_mlp(bs, seq_len)  / self.tp_size
        elif ops_type=="rmsnorm":
            weight_memory = 2*self.llm_params.count_params_per_layer_rn()   # rmsnorm has no matrix weight, only vector weight, is ignored
            flops = self.llm_flops.count_flops_per_layer_rmsnorm(bs, seq_len)   # rmsnorm is not compute bound, flops is very small
            memory_access = self.llm_memory.count_memory_access_per_layer_rmsnorm(bs, seq_len) # memory access
        else:    
    ...
    def count_latency_per_layer_tp_comm(self, bs: int, seq_len: int) -> float:
        ...
        num_data_per_all_reduce = 6 * bs * seq_len * self.h * (self.tp_size - 1) /(self.tp_size)

        latency_per_layer_tp_comm = num_data_per_all_reduce * self.bytes_per_param / self.gpu_intra_node_bandwidth
    ...
    def count_latency_per_layer(
        ...
        latency_per_layer_qkvo_proj = self.common_count_latency_for_ops(bs, seq_len, is_inference, act_recomputation, ops_type="qkvo_proj")
        latency_per_layer_attn_kernel = self.common_count_latency_for_ops(bs, seq_len, is_inference, act_recomputation, ops_type="attn_kernel")

        latency_per_layer_mlp = self.common_count_latency_for_ops(bs, seq_len, is_inference, act_recomputation, ops_type="mlp")
        latency_per_layer_rmsnorm = self.common_count_latency_for_ops(bs, seq_len, is_inference, act_recomputation, "rmsnorm")
    ...
    def count_latency_kv_cache_per_layer(
        ...
        kv_cache_memory_access = self.llm_memory.count_memory_access_per_layer_kv_cache(
            bs, seq_len, generate_len,
            flash_attn, kv_cache_bytes
        ) / self.tp_size
    ...
    def count_latency_model(
        ...
        model_latency_breakdown = {
            breakdown_prefix + "qkvo_proj": (breakdown_per_layer["qkvo_proj"] * num_layers_per_gpu),
            breakdown_prefix + "attn_kernel": (breakdown_per_layer["attn_kernel"] * num_layers_per_gpu),
            breakdown_prefix + "mlp": (breakdown_per_layer["mlp"] * num_layers_per_gpu),
            breakdown_prefix + "rmsnorm": (breakdown_per_layer["rmsnorm"] * num_layers_per_gpu),
            breakdown_prefix + "tp_comm": (breakdown_per_layer["tp_comm"] * num_layers_per_gpu),
            breakdown_prefix + "kv_cache_rw": (breakdown_per_layer["kv_cache_rw"] * num_layers_per_gpu),
        }
