...
class CountCausalLMFlops(object):
    ...
    def __init__(self, model_config: ModelConfig, bs: int, seq_len: int, simp_count=False) -> None:
    ...
    def count_flops_per_layer_qkvo_proj(self, bs: int, seq_len: int) -> int:
        ...
        """
        qkvo_proj_flops = 4 * 2* bs * seq_len * self.hidden_size * self.hidden_size
        
        return qkvo_proj_flops
    
    def count_flops_per_layer_attention_kernel(self, bs: int, seq_len: int) -> int:
    ...
    def count_flops_per_layer(self, bs: int, seq_len: int, ln_ignore=True) -> tuple:
        ...
        flops_per_layer_attention_kernel = self.count_flops_per_layer_attention_kernel(bs, seq_len)
        flops_per_layer_rmsnorm = self.count_flops_per_layer_rmsnorm(bs, seq_len) * 2 # atten_rmsnorm mlp_rmsnorm
        ...
        dict_flops_per_layer = {
            "flops_attention_kernel": flops_per_layer_attention_kernel,
            "flops_qkvo_proj": flops_per_layer_qkvo_proj,
            "flops_mlp": flops_per_layer_mlp,
            "flops_rmsnorm": flops_per_layer_rmsnorm * 2,
        }
    ...
    def count_flops_logits_layer(self,) -> int:
        ...
        return 2 * self.b * self.s * self.hidden_size * self.V
